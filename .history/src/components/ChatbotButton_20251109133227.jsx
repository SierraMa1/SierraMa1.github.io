...
export default function ChatbotButton() {
  ...
  // Configuración - Conexión a través del Proxy de Next.js
  const BOTPRESS_ID = process.env.NEXT_PUBLIC_BOTPRESS_ID || "mariasierradev-bot";
  
  // Apunta a tu ruta de API que actúa como proxy
  ...
  const handleScriptLoad = () => {
    console.log('✅ Script de Botpress cargado (vía proxy)');
    
    if (window.botpressWebChat) {
      try {
        // Inicializar con el widget VISIBLE por defecto
        window.botpressWebChat.init({
          hostUrl: BOTPRESS_HOST, // <-- ¡MUY IMPORTANTE! Usa la ruta del proxy
          botId: BOTPRESS_ID,
          hideWidget: false,
          showPoweredBy: false,
        ...
        setIsBotReady(true);
        console.log('✅ Botpress inicializado correctamente (vía proxy)');
        console.log('🔗 Host URL (proxy):', BOTPRESS_HOST);
  ...
  const handleScriptError = (error) => {
    console.error('❌ Error al cargar el script (vía proxy):', error);
  };
  ...
  return (
    <>
      {/* Script de Botpress - Conexión vía proxy */}
      <Script 
        src={SCRIPT_SRC}
