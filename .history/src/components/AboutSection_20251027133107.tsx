...
import { useState, useEffect } from 'react'; // Imports para hooks
import { Star, GitFork, GitCommit } from 'lucide-react'; // Iconos para el widget

//Defino el tipo de dato que espero de la API
interface Repo {
  id: number;
  name: string;
  html_url: string;
...
export default function AboutSection() {
  // Estado para guardar el repositorio más reciente
  const [latestRepo, setLatestRepo] = useState<Repo | null>(null);
  const [loading, setLoading] = useState(true);

  // Hook para llamar a la API de GitHub al cargar
  useEffect(() => {
    async function fetchLatestRepo() {
      try {
        // Sólo el último repositorio actualizado del usuario 'SierraMa1'
        ...
        if (data.length > 0) {
          setLatestRepo(data[0]); // Se guarda primer (y único) repo en el estado
        }
      } catch (error) {
      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 items-center gap-12 md:grid-cols-2 lg:gap-16">
          ...
          <div className="flex justify-center">
            {/*  'fill' y 'sizes' para arreglar el warning de 'yo.png' */}
          ...
          <div className="text-center md:text-left mt-8 md:mt-0">
            ...
            {/* El nuevo "Widget de Actividad en Vivo" */}
            <div className="mt-8 rounded-lg border border-gray-200 bg-gray-50 p-6">
              ...
              {!loading && latestRepo && (
                <div>
                  ...
                  {/* CAMBIO: Texto "Sin descripción" con estilo cursiva y más claro */}
                  <p className="text-sm text-gray-600 mb-2 truncate" title={latestRepo.description || "Sin descripción"}>
